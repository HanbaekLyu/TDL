...
# Author: Hanbaek Lyu
# Reference: Hanbaek Lyu, Deanna Needell, Laura Balzano,
...
class Online_NMF():
    ...
    def __init__(self,
        ...
        After matricizing X, seeks to find nonnegative matrix factorization X \approx \sum W^l * H^l
        W = (d by r), H = (n by r)
    ...
    def joint_sparse_code_tensor(self, X, W, alpha=0, nonnegativity=True):
        '''
        ...
        coder = SparseCoder(dictionary=W.T, transform_n_nonzero_coefs=None,
                            transform_alpha=alpha, transform_algorithm='lasso_lars', positive_code=nonnegativity)
    ...
    def update_dict(self, W, A, B, nonnegativity=True, sparsity=0):
        '''
        ...
        for j in np.arange(r):
            ...
            W1[:, j] = W1[:, j] - (1 / (A[j, j] + 1)) * (np.dot(W1, A[:, j]) - B.T[:, j] + sparsity * np.ones(W1.shape[0]))
    ...
    def step(self, X, A, B, W, t, nonnegative_dict=True, nonnegative_code=True, dict_sparsity=0, code_sparsity=0):
        '''
        ...
        H1 = self.joint_sparse_code_tensor(X, W, alpha=code_sparsity, nonnegativity=nonnegative_code)
        ...
        W1 = self.update_dict(W, A, B, nonnegativity=nonnegative_dict, sparsity=dict_sparsity)
        self.history = t + 1
        # print('history=', self.history)
        return H1, A1, B1, W1

    def train_dict_single(self, nonnegative_dict=True, nonnegative_code=True, dict_sparsity=0, code_sparsity=0):
        '''
        ...
        for i in np.arange(1, self.iterations):
            ...
            if self.subsample:
                ...
                H, A, B, W = self.step(X_batch, A, B, W, t0 + i,
                                       ...
                                       nonnegative_code=nonnegative_code,
